...
from python_packages.simple_dd import CreateBernoulli, CreateElectronCurrent, CreateHoleCurrent
...
def PrintCurrents(device, contact):
    ...
    print(f"{contact}\t{voltage}\t{electron_current}\t{hole_current}\t{total_current}")

def ReturnCurrents(device, contact):
    """
    return out contact currents
    Added 05/29/25 by DJulson
    """
    # TODO add charge
    _contact_bias_name = GetContactBiasName(contact)
...
def SetNitrideParameters(device, region, T):
    """
    Sets physical parameters
    """
    set_parameter(device=device, region=region, name="Permittivity", value=eps_nitride * eps_0)
    set_parameter(device=device, region=region, name="ElectronCharge", value=q)
...
# def CreateNitridePotentialOnly(device, region, update_type="default"):
#     """
#     Create electric field model in oxide
#     Creates Potential solution variable if not available
#     """
...
def CreateNitridePotentialOnly(device, region, update_type="default"):
    """
    Create electric field model in nitride
    Includes effect of trapped charge as node model 'TrappedCharge'
    """
    ...
    # Define electric field and displacement field
    efield = "(Potential@n0 - Potential@n1)*EdgeInverseLength"
    CreateEdgeModel(device, region, "ElectricField", efield)
    CreateEdgeModelDerivatives(device, region, "ElectricField", efield, "Potential")
    ...
    # ---- NEW: Define trapped charge model ----
    # Assumes you have defined a 'trap_density' parameter elsewhere
    if not InNodeModelList(device, region, "TrappedCharge"):
        CreateNodeModel(device, region, "TrappedCharge", "-ElectronCharge * Electrons")

    # ---- Add RHS charge term into Poisson’s equation ----
    equation(
        device=device,
        region=region,
        name="PotentialEquation",
